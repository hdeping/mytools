...
class LanNet(nn.Module):
    ...
    def forward(self, src, mask, target):
        ...
        # get the loss
        loss_pitch,predict_target = self.getLoss(out_hidden_pitch,mask,target,fea_frames)
        loss_fb   ,predict_target = self.getLoss(out_hidden_fb,mask,target,fea_frames)
        
        ce_loss = loss_pitch*self.alpha + loss_fb*(1 - self.alpha)

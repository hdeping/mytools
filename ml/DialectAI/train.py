...
for piece in range(0,16):
    ...
    modelfile = '%s/model%d.model'%(model_dir, piece)
    torch.save(train_module.state_dict(), modelfile)
    epoch_toc = time.time()
    piece_time = epoch_toc-epoch_tic
    #logging.info('Piece:%d, train-acc:%.6f, train-loss:%.6f, cost time :%.6fs', piece, train_acc/sum_batch_size, train_loss/sum_batch_size, epoch_time)
    logging.info('Piece:%d,cost time :%.6fs', piece,piece_time)
...
##  dev
    ...
    logging.info('Piece:%d, dev-acc:%.6f, dev-loss:%.6f, cost time :%.6fs', piece, acc, dev_loss/dev_batch_num, epoch_time)
